class HashTable
{
   ...
   int hashFunction(PVector pos)
{
   return hashCoords(int(pos.x / _cellSize), int(pos.y / _cellSize));
}

  int hashCoords(int cx, int cy)
{

   int h = (cx * 92837111) ^ (cy * 689287499);
   return abs(h) % _numCells;
}
}
