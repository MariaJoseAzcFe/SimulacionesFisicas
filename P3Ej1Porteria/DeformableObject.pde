import java.util.TreeMap;

public class DeformableSurface 
{
  float _lengthX;  
  float _lengthY;   
  
  int _numNodesX;  
  int _numNodesY;   

  SpringLayout _springLayout; 
  boolean _isUnbreakable;  
  color _color;  

  Particle[][] _nodes;   
  ArrayList<DampedSpring> _springsSurface;  
  TreeMap<String, DampedSpring> _springsCollision;   


  DeformableSurface(float lengthX, float lengthY, int numNodesX, int numNodesY, float surfacePosZ, float nodeMass, float Ke, float Kd, float maxForce, float breakLengthFactor, SpringLayout springLayout, boolean isUnbreakable, color c)
  {
    _lengthX = lengthX;
    _lengthY = lengthY;

    _numNodesX = numNodesX;
    _numNodesY = numNodesY;
    
    _springLayout = springLayout;
    _isUnbreakable = isUnbreakable;
    _color = c;

    _nodes = new Particle[_numNodesX][_numNodesY];
    _springsSurface = new ArrayList();
    _springsCollision = new TreeMap<String, DampedSpring>();

    createNodes(surfacePosZ, nodeMass);
    createSurfaceSprings(Ke, Kd, maxForce, breakLengthFactor);
  }
  

  void createNodes(float surfacePosZ, float nodeMass)
  {
        
     for (int i = 0; i < _numNodesX; i++)
      {
        for (int j = 0; j < _numNodesY; j++) {
  
          PVector pos = new PVector ((i*_lengthX/_numNodesX) - _lengthX/2, (j*_lengthY/_numNodesY) - _lengthY/2, surfacePosZ);
  
          PVector vel = new PVector (0, 0, 0);
          boolean clamped = false;
  
          if ((i==0) || (j==0) || (i == _numNodesX -1) || (j == _numNodesY-1)) //Dejamos fijos los bordes de la malla
          {  
            clamped = true;
          }
          _nodes[i][j] = new Particle(pos, vel, nodeMass, clamped);
        }
      }   
  }
  


  void createSurfaceSprings(float Ke, float Kd, float maxForce, float breakLengthFactor)
  {

     switch(_springLayout)
     {
       case STRUCTURAL:
       
         for (int i = 0; i< _numNodesX; i++)
         {
           for (int j = 0; j< _numNodesY; j++)
           {
             boolean repulsion = false; //Serán repulsivos y atrayentes porque la malla debe recuperar su estado inicial 
                                         //cuando se estire (y no se haya roto)
             
             if(i < _numNodesX-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             
             if(j < _numNodesY-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           }
         }

       break;
       
       
       
       
       case SHEAR:
       
         for (int i = 0; i< _numNodesX-1; i++)
         {
           for (int j = 0; j< _numNodesY-1; j++)
           {
             
             boolean repulsion = false;
             
             _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             _springsSurface.add(new DampedSpring(_nodes[i][j+1], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           }
         }
       
       break;
      
       
       
       
       case STRUCTURAL_AND_SHEAR:
       
       
        for (int i = 0; i< _numNodesX; i++)
         {
           for (int j = 0; j< _numNodesY; j++)
           {
             //STRUCTURAL
             boolean repulsion = false; 
             
             if(i < _numNodesX-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             
             if(j < _numNodesY-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           
             //SHEAR
             if(i < _numNodesX - 1 && j < _numNodesY - 1){
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
               _springsSurface.add(new DampedSpring(_nodes[i][j+1], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           
             }
           }
         }      
       break;
       
       case STRUCTURAL_AND_BEND:
        
        //STRUCTURAL
        for (int i = 0; i< _numNodesX; i++)
         {
           for (int j = 0; j< _numNodesY; j++)
           {
             
             boolean repulsion = false; 
             
             //STRUCTURAL
             if(i < _numNodesX-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             
             if(j < _numNodesY-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
            
           
             //BEND
             if(i < _numNodesX-2)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+2][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             
             if(j < _numNodesY-2)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i][j+2], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           
           }
         }   
       break;
       
       
       case STRUCTURAL_AND_SHEAR_AND_BEND:
       

       for (int i = 0; i< _numNodesX; i++)
         {
           for (int j = 0; j< _numNodesY; j++)
           {
             
             boolean repulsion = false; 
             
             
             //STRUCTURAL
             if(i < _numNodesX-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             
             if(j < _numNodesY-1)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           
           
             //SHEAR
             if(i < _numNodesX - 1 && j < _numNodesY - 1){
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+1][j+1], Ke, Kd, repulsion, maxForce, breakLengthFactor));
               _springsSurface.add(new DampedSpring(_nodes[i][j+1], _nodes[i+1][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           
             }
             
             //BEND
             if(i < _numNodesX-2)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i+2][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             
             if(j < _numNodesY-2)
               _springsSurface.add(new DampedSpring(_nodes[i][j], _nodes[i][j+2], Ke, Kd, repulsion, maxForce, breakLengthFactor));
           }
         } 
       break;
       
     }
     
  }

  void update(float simStep)
  {
    int i, j;

    for (i = 0; i < _numNodesX; i++)
      for (j = 0; j < _numNodesY; j++)
        if (_nodes[i][j] != null)
          _nodes[i][j].update(simStep);

    for (DampedSpring s : _springsSurface) 
    {
      s.update(simStep);
      s.applyForces();
    }

    for (DampedSpring s : _springsCollision.values()) 
    {
      s.update(simStep);
      s.applyForces();
    }
  }



  void avoidCollision(Ball b, float Ke, float Kd, float maxForce, float breakLengthFactor)
  {
     boolean repulsion = true;
      for (int i = 0; i< _numNodesX; i++)
         {
           for (int j = 0; j< _numNodesY; j++)
           {
             if (_nodes[i][j]._s.dist(b._s) < b._r)  
             {
               if(!_springsCollision.containsKey(_nodes[i][j].getId() + "," +b.getId()))
                 _springsCollision.put(_nodes[i][j].getId() + "," +b.getId(), new DampedSpring(b, _nodes[i][j], Ke, Kd, repulsion, maxForce, breakLengthFactor));
             }
             else
             {
               _springsCollision.remove(_nodes[i][j].getId() + "," +b.getId());
             }
           }
         }     
  }

  void draw()
  {
    if (_isUnbreakable) 
       drawWithQuads();
    else
       drawWithSegments();
  }

  void drawWithQuads()
  {
    int i, j;
    
    fill(255);
    stroke(_color);

    for (j = 0; j < _numNodesY - 1; j++)
    {
      beginShape(QUAD_STRIP);
      for (i = 0; i < _numNodesX; i++)
      {
        if ((_nodes[i][j] != null) && (_nodes[i][j+1] != null))
        {
          PVector pos1 = _nodes[i][j].getPosition();
          PVector pos2 = _nodes[i][j+1].getPosition();

          vertex(pos1.x, pos1.y, pos1.z);
          vertex(pos2.x, pos2.y, pos2.z);
        }
      }
      endShape();
    }
  }

  void drawWithSegments()
  {
    stroke(_color);

    for (DampedSpring s : _springsSurface) 
    {
      if (!s.isBroken())
      {
        PVector pos1 = s.getParticle1().getPosition();
        PVector pos2 = s.getParticle2().getPosition();

        line(pos1.x, pos1.y, pos1.z, pos2.x, pos2.y, pos2.z);
      }
    }
  }
}
